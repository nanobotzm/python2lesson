...
from test_check.models import City, Language
...
class UserLoginForm(forms.Form):
    ...
    def clean(self, *args, **kwargs):
        ...
        if email and password:
            ...
            if not qs.exists():
                raise forms.ValidationError('Данный пользователь не зарегистрирован.')
...
class UserRegistrationForm(forms.ModelForm):
    ...
    password2 = forms.CharField(label='Плдтсвердите пароль пароль',
                                widget=forms.PasswordInput(attrs={'class': 'form-control'}))
...
class ContactForm(forms.Form):
    ...
    language = forms.CharField(required=True,
                               widget=forms.TextInput(attrs={'class': 'form-control'}),
                               label='Вакансия')
    email = forms.CharField(required=True, label='Введите email',
                            widget=forms.EmailInput(attrs={'class': 'form-control'}))
