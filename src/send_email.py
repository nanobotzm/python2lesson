...
from test_check.models import Vacancy, Errors, URL
...
if users_dct:
    ...
    for keys, emails in users_dct.items():
        ...
        for row in rows:
            ...
            html += f'<p><b> {row["company"]} </b></p><br><hr>'
...
if qs.exists():
    ...
    subject += f'Ошибки сбора вакансий {today}. '
    text_content += 'Ошибки'
...
for keys in users_dct.keys():
    if keys not in urls_dct:
        if keys[0] and keys[1]:
            urls_errors += f'<p>Для города: {keys[0]}, ЯП: { keys[1] } отсутствуют ссылки.</p>'

if urls_errors:
    subject += 'Отсутсвующие ссылки.'
    _html += '<hr>'
    _html += '<h2>Отсутсвтующие ссылки.</h2>'
    _html += urls_errors
if subject:
    msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
    msg.attach_alternative(_html, "text/html")
